	...
	public void validate(Employee employee) throws Exception {
		...
		try {
			if(employee.getName()==null || employee.getName().isBlank())
				throw new Exception(config.getProperty("Validator.INVALID_EMPLOYEE_NAME").toString());
			if( employee.getId()<1000)
				throw new Exception(config.getProperty("Validator.INVALID_EMPLOYEE_ID").toString());
		}
